...
struct WorldOcclusionView: View {
    ...
    var body: some View {
        RealityView { content in
            ...
            let sphereMesh = MeshResource.generateSphere(radius: 0.2) // Adjust radius as needed
            ...
            // Generate collision shapes for the sphere.
            sphere.generateCollisionShapes(recursive: true)

            // Enable inputs to detect the hand gestures.
            sphere.components.set(InputTargetComponent())

            // Add environment blending for occlusion.
            sphere.components.set(
                EnvironmentBlendingComponent(preferredBlendingMode: .occluded(by: .surroundings))
            )

//             Add the sphere entity to the `RealityView`.
            content.add(sphere)
        }
